...
pkgTest <- function(x, bc = FALSE) {
  ...
  # Sürüm kontrolü sırasında tür hatalarını önleyin
  version <- packageVersion(x)
  if (!is.character(as.character(version))) {
    stop(paste0("Invalid package version format for ", x, ": ", version))
  }
}
